...
class ObservationSession:
    """Hot observation session with latest-frame cache and causal actions."""

    def __init__(self, client: ObservationClient) -> None:
        self.client = client
        self._latest_frame: ObservationFrame | None = None

    @property
    def latest_frame(self) -> ObservationFrame | None:
    ...
    def start(self, *, drain_initial_frame: bool = True) -> ObservationFrame | None:
        frame = self.client.start(drain_initial_frame=drain_initial_frame)
        if frame is not None:
            self._cache_frame(frame)
        return self._latest_frame

    def receive_next(self) -> ObservationFrame:
        self.start(drain_initial_frame=True)
        frame = self.client.transport.receive_frame(
            transport_timing=bool(self.client.payload.get("transport_timing"))
        )
        self._cache_frame(frame)
        return frame

    def observe_latest(self, *, request_frame: bool = False) -> ObservationFrame | None:
        self.start(drain_initial_frame=True)
        if request_frame or self._latest_frame is None:
    ...
    def act_and_observe(
        ...
        *,
        ...
        self.start(drain_initial_frame=True)
        result = self.client.act_and_observe(
            actions=actions,
            source=source,
            capture_delay_ms=capture_delay_ms,
            change_timeout_ms=change_timeout_ms,
        ...
        self._cache_frame(result.frame)
        return result

    def _cache_frame(self, frame: ObservationFrame) -> None:
        if self._latest_frame is None or frame.seq >= self._latest_frame.seq:
