...
_REQUIRED_EWMH_REQUESTS = ("_NET_ACTIVE_WINDOW", "_NET_CLOSE_WINDOW")
...
class NativeEwmhWindowAdapter:
    ...
    def available(self) -> bool:
        try:
            with self._lock:
                self._ensure_open()
                self._require_live_window_manager()
                self._client_window_ids()
                self._require_supported_operations()
    ...
    def _require_live_window_manager(self) -> None:
        assert self._root is not None
        root_check = self._property_values(
            self._root,
            "_NET_SUPPORTING_WM_CHECK",
            expected_type_name="WINDOW",
        )
        if not root_check or root_check[0] == 0:
            raise X11WindowNativeUnavailableError(
                "window manager does not expose _NET_SUPPORTING_WM_CHECK"
            )
        manager_window = root_check[0]
        manager_check = self._property_values(
            manager_window,
            "_NET_SUPPORTING_WM_CHECK",
            expected_type_name="WINDOW",
        )
    ...
    def _require_supported_operations(self) -> None:
        assert self._root is not None
        supported = self._property_values(
            self._root,
            "_NET_SUPPORTED",
...
class X11WindowController:
    ...
    async def list(self) -> list[X11Window]:
        try:
            ...
            if not self._commands.available():
                ...
                return await self._commands.fallback_list()
            self._backend_name = "wmctrl"
    ...
    async def activate(self, window_id: str) -> ActionResult:
        ...
        try:
            ...
            if not self._commands.available():
                ...
                return _window_command_unavailable_result(normalized_id, "activate")
            self._backend_name = "wmctrl"
    ...
    async def close_window(self, window_id: str) -> ActionResult:
        ...
        try:
            ...
            if not self._commands.available():
                self._backend_name = "unavailable"
                return _window_command_unavailable_result(normalized_id, "close")
            self._backend_name = "wmctrl"
