...
class X11KeyboardController:
        ...
        async with self._operation_lock:
            ...
            if self._native_enabled():
                try:
                    ...
                    owned = await self._native_key_down(stroke)
                    held = _HeldKey(keycode=stroke.keycode, owned=owned)
            else:
                self._active_backend = "xdotool"
                await self._run("xdotool", "keydown", normalized)
                held = _HeldKey(keycode=None, owned=True)
    ...
    async def up(self, key: str) -> None:
        async with self._operation_lock:
            ...
            if tracked is not None and tracked.keycode is not None:
                if tracked.owned:
                    self._xtest.emit([KeyEvent(tracked.keycode, False)])
    ...
    async def _native_key_down(self, stroke: _KeyStroke) -> bool:
