...
def test_click_beacon_case_reports_missing_dom_click_events(monkeypatch) -> None:
    ...
    assert result["click_index_events_before_actions"] == 1
    assert result["click_index_events_after_actions"] == 1
...
def test_click_target_state_case_reports_window_state(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        observation_surface,
        "_probe_click_page_server",
        lambda _client, _beacon_id: {"ok": True, "status": 200},
    )
    monkeypatch.setattr(
        observation_surface,
...
def test_serve_synthetic_page_uses_socket_probe_for_http_server_guard() -> None:
    ...
    observation_surface._serve_synthetic_page(
...
def test_open_click_toggle_beacon_page_installs_click_beacon(monkeypatch) -> None:
    ...
    assert token in str(open_payload["url"])
    assert calls[2] == (
        "sleep",
        observation_surface.CLICK_TOGGLE_PAGE_READY_SETTLE_MS / 1000,
    )
...
def test_read_click_index_count_parses_command_stdout() -> None:
    calls: list[tuple[str, dict[str, object]]] = []
    ...
    count = observation_surface._read_click_index_count(
        FakeClient(),  # type: ignore[arg-type]
        "token with spaces",
    )
    ...
    assert path == "/v1/commands/run"
    assert isinstance(payload["command"], list)
    assert "GET /index.html?action-observe-beacon=token%20with%20spaces" in str(
        payload["command"]
    )
...
def test_probe_click_page_server_parses_command_stdout() -> None:
    calls: list[tuple[str, dict[str, object]]] = []

    class FakeClient:
        def post_json(self, path: str, *, json: dict[str, object]) -> dict[str, object]:
            calls.append((path, json))
            return {"ok": True, "output": {"stdout": "ok=true\nstatus=200\nbytes=42\n"}}

    result = observation_surface._probe_click_page_server(
        FakeClient(),  # type: ignore[arg-type]
        "token with spaces",
    )

    assert result == {"ok": True, "status": 200, "bytes": 42}
    path, payload = calls[0]
    assert path == "/v1/commands/run"
    assert isinstance(payload["command"], list)
...
def test_wait_for_click_ready_count_polls_until_expected(monkeypatch) -> None:
    counts = iter([0, 1])
    sleeps: list[float] = []
    ...
    monkeypatch.setattr(
        observation_surface,
        "_read_click_ready_count",
        lambda _client, _token: next(counts),
    )
    monkeypatch.setattr(observation_surface.time, "sleep", lambda seconds: sleeps.append(seconds))

    count = observation_surface._wait_for_click_ready_count(
        object(),  # type: ignore[arg-type]
        "token",
        expected_events=1,
        timeout_ms=500,
...
def test_paired_dirty_producer_case_reports_delta_samples(monkeypatch) -> None:
    ...
    action_region_summary = result["variant"]["dirty_frame_capture_region_source_summaries"][
        "action_region"
    ]
    assert action_region_summary["frames"] == 1
    ...
    assert xdamage_summary["frames"] == 1
    assert xdamage_summary["producer_used_frames"] == 1
    assert xdamage_summary["changed_frames"] == 0
    assert xdamage_summary["dirty_frame_age_summary_ms"]["p50"] == 0.1
...
def test_paired_timeout_case_compares_change_timeout_budget(monkeypatch) -> None:
    monkeypatch.setattr(observation_surface, "_open_click_toggle_page", lambda _client: None)
    captured: dict[str, object] = {}
    ...
    def fake_measure_paired_stream_action_observe_loop(**kwargs):
        captured.update(kwargs)
        return {
            "baseline_samples_ms": [105.0],
            "variant_samples_ms": [42.0],
            "baseline_observations": [
                {
                    "change_timeout_reached": True,
                    "benchmark_arm": {
                        "change_timeout_ms": kwargs["baseline_change_timeout_ms"],
                    },
                }
            ],
            "variant_observations": [
                {
                    "change_timeout_reached": False,
                    "benchmark_arm": {
                        "change_timeout_ms": kwargs["variant_change_timeout_ms"],
                    },
                }
            ],
            "paired_delta_samples_ms": [-63.0],
            "paired_observations": [],
        }
    ...
    result = observation_surface._run_observation_action_click_paired_timeout_benchmark(
        base_url="http://daemon.test",
        token=None,
        client=object(),  # type: ignore[arg-type]
        iterations=1,
    ...
    assert captured["baseline_change_timeout_ms"] == 100
    assert captured["variant_change_timeout_ms"] == 200
    assert captured["change_detection"] == "auto"
    assert captured["order_seed"] == observation_surface.PAIRED_TIMEOUT_ORDER_SEED
    assert result["baseline"]["change_timeout_ms"] == 100
    assert result["variant"]["change_timeout_ms"] == 200
    assert result["paired_comparison"]["variant_wins"] == 1
    assert result["pairing"]["scope"] == (
        "same sandbox/client path/page/stream, per-command timeout budget"
    )
...
def test_paired_dirty_region_confirmation_case_compares_confirmation_policy(
    monkeypatch,
) -> None:
    monkeypatch.setattr(observation_surface, "_open_click_toggle_page", lambda _client: None)
    captured: dict[str, object] = {}
    ...
    def fake_measure_paired_stream_action_observe_loop(**kwargs):
        ...
        return {
            "baseline_samples_ms": [12.0],
            "variant_samples_ms": [8.0],
            "baseline_observations": [
                {
                    "dirty_region_confirmation_result": "unchanged",
                    "change_stage_timing_ms": {
                        "dirty_region_confirmation_capture_ms": 4.0,
                        "server_pre_emit_ms": 10.0,
                    },
                }
            ],
            "variant_observations": [
    ...
    result = (
        observation_surface._run_observation_action_click_paired_dirty_region_confirmation_benchmark(
            base_url="http://daemon.test",
            token=None,
            client=object(),  # type: ignore[arg-type]
            iterations=1,
    ...
    assert captured["baseline_dirty_region_confirmation"] == "auto"
    assert captured["variant_dirty_region_confirmation"] == "off"
    assert captured["baseline_change_region_radius"] == 64
    assert captured["variant_change_region_radius"] == 64
    assert captured["order_seed"] == observation_surface.PAIRED_DIRTY_REGION_CONFIRMATION_ORDER_SEED
    assert result["baseline"]["dirty_region_confirmation"] == "auto"
    assert result["variant"]["dirty_region_confirmation"] == "off"
    assert (
        result["baseline"]["change_stage_timing_summary_ms"][
            "dirty_region_confirmation_capture_ms"
        ]["p50"]
        == 4.0
    )
...
def test_frame_observations_summarize_dirty_frame_producer_metadata() -> None:
    ...
    assert source_summary["dirty_region_confirmation_results"] == ["unchanged"]
    assert source_summary["dirty_frame_age_summary_ms"]["p50"] == 0.3
