...
**v6 delta:** v6 keeps the v5 product architecture but makes the repository **UV-first**. Local development, dependency resolution, editable installs, lockfile management, CI commands, docs, and examples should use `uv` instead of raw `pip`. The repository should check in `uv.lock`, document `uv sync --extra dev` / `uv run ...` commands, and avoid `python -m pip install ...` in project-owned instructions. Modal image helpers may still call Modal SDK methods named `pip_install` because that is Modal's image API surface, but package dependency management and developer workflow should be UV-based.
...
| Packaging and tooling | Use UV instead of raw pip for repo installs, lockfiles, and commands. | UV gives faster, reproducible dependency management and a single command surface for CI/local workflows. | Check in `uv.lock`; document `uv sync --extra dev`, `uv run pytest`, `uv run ruff check .`, and `uv run mypy src`; update README, AGENTS, examples, and CI away from `python -m pip install ...`. |
...
- **`modal.Image`**: define the desktop image as code with apt packages, Python packages, local package source, environment variables, and entrypoint.
...
image = default_image(
    ...
    extra_apt=["libnss3"],
    extra_python_packages=["playwright"],
    extra_run_commands=[],
)
```
...
Exact Modal image code should follow Modal's current `modal.Image` APIs (`debian_slim`, `apt_install`, `pip_install`, `add_local_python_source`, `add_local_dir`, `env`, `cmd`, etc.). The method name `pip_install` is a Modal SDK image-construction API; it does not change the repository-level decision to use UV for local development, lockfiles, installs, and CI commands.
...
- Keep text editor and bash adapters in optional extras, for example `uv add "modal-computer-use[anthropic]"` for downstream projects or `uv sync --extra anthropic` inside this repository.
...
- A prebuilt Modal image with all apt and Python dependencies.
...
- Python package installs cleanly through UV, with `uv sync --extra dev` for this repository and `uv add modal-computer-use` for downstream consumers.
