...
From my laptop the move-and-click took 36.7 ms. From the Function it took 4.7 ms.

The daemon itself did about a millisecond of work in both cases. The trip to the desktop and back took 35 ms from my laptop and 3.4 ms from the Function.

A screenshot went from 84.8 ms to 39.5 ms in the same comparison. The click gained more because a click request carries almost nothing in either direction, so nearly all of its cost is the trip. A screenshot carries a PNG back, and the desktop spent 23 ms capturing and encoding it whether the client ran on my laptop or in the Function.
