...
## Why did one shell command in twenty take four times as long?

With screenshots and clicks down in the tens of milliseconds, the shell-command endpoint started to look wrong. Its median was 55 ms and its p95 was 220 ms, and the slow requests were rare and severe: two samples out of thirty carried that entire tail. A distribution shaped like that usually means something is waiting in line.
...
Over 30 samples per arm, the private loop measured 7.6 ms p50 and 8.7 ms p95.

A thread pool got most of the way there, at 10.6 ms and 13.2 ms, and that is why the sample count mattered. At ten samples per arm the thread pool and the private loop were indistinguishable, so my choice between them had been a coin flip I did not know I was tossing. At thirty they separate, and the private loop wins by about 3 ms at the median and 4.5 ms at the tail.
