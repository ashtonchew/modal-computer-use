...
| `actions.screenshot_capture_source` | `"auto"` | `"auto"`, `"mss"`, or `"x11-shm"`; maps to `COMPUTER_USE_SCREENSHOT_CAPTURE_SOURCE`. `auto` selects X11 shared-memory capture only after its extension and live display pass readiness, then keeps MSS as a sticky fallback for that daemon session. Explicit `x11-shm` fails readiness instead of silently selecting MSS. |
...
| `COMPUTER_USE_SCREENSHOT_CAPTURE_SOURCE` | `auto` | Screenshot source selector: `auto`, `mss`, or `x11-shm`. `auto` uses X11 shared-memory capture when the managed Image and live display support it, otherwise it selects MSS once for the daemon session. Explicit `x11-shm` fails readiness when unavailable. |
