...
| Cursor-hidden capture | In-process MSS | `scrot`, then `maim` | After a bounded MSS reset/retry cannot return a valid frame. |
...
Cursor-hidden MSS capture is preferred. Cursor-visible capture uses the compatibility path because
the native capture path does not compose the cursor.
