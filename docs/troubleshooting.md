...
window manager, the screenshot pipeline, and noVNC when enabled. `xdotool` is required only when
the configured input adapter or an automatic fallback needs it; `wmctrl` is required only when the
native EWMH window adapter is unavailable. Hit `/readyz` directly and read the per-check failure
list.
